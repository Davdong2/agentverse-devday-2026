...
export async function GET(){
 if(cached&&Date.now()-Date.parse(cached.fetchedAt)<1200000)return Response.json({...cached,mode:'cached'},{headers:{'Cache-Control':'private, max-age=60'}});
 try{
 ...
 cached=data;return Response.json({...data,mode:'fresh'},{headers:{'Cache-Control':'private, max-age=60'}});
