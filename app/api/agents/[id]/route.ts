...
export async function GET(
  ...
  try {
    ...
    const s = p?.AgentDetailPage?.services;
    if (!Array.isArray(s?.list)) throw new Error('No services');
    const data: Detail = {
    ...
    cached.set(id, data);
    return Response.json({ ...data, mode: 'fresh' });
  } catch {
    const saved = (details as Record<string, Detail>)[id];
    if (old || saved)
      return Response.json({ ...(old ?? saved), mode: 'snapshot' });
    return Response.json(
      { error: '服务资料暂不可用，请查看 OKX.AI 原始页面。' },
      { status: 503 },
    );
  }
}
