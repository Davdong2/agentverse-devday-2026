...
- Listing review: **two reviews completed on 19 September 2026 with the same
  delivery finding**. The second review exposed an HTTP 415 response to the
  official-style empty `POST` availability probe; that path is corrected before
  the next resubmission. Do not claim approval until written confirmation exists
