...
const agents = [
  ...
  {
    agentId: '3', name: 'Execution Agent', avatar: '', description: 'X Layer 交易执行准备',
    approvalRate: '100%', usageCount: 8, startingPrice: '0', symbol: 'USDT',
    categories: ['TRADING'], categoryName: ['交易'], onlineStatus: 1,
  },
  {
    agentId: '4', name: 'Food Agent', avatar: '', description: '家庭饮食和菜谱',
    approvalRate: '100%', usageCount: 100, startingPrice: '0', symbol: 'USDT',
    categories: ['LIFE'], categoryName: ['生活'], onlineStatus: 1,
  },
];
const catalog = {
  agents,
  details: {
    ...
    '3': { fetchedAt: '2026-09-17T00:00:00Z', total: 1, services: [{ serviceId: 33, name: '交易执行准备', description: '生成 X Layer swap 交易参数，不自动签名', price: '0', symbol: 'USDT', serviceType: 'A2MCP' }] },
    '4': { fetchedAt: '2026-09-17T00:00:00Z', total: 1, services: [{ serviceId: 44, name: '家庭菜谱', description: '生成一周饮食计划', price: '0', symbol: 'USDT', serviceType: 'A2MCP' }] },
  },
...
test('mission planner enforces input and agent limits', () => {
  ...
  assert.throws(() => composeMission({ goal: '分析市场新闻', maxAgents: 99 }, catalog), /1 到 4/);
  assert.throws(() => composeMission({ goal: '分析市场新闻', maxAgents: 0 }, catalog), /1 到 4/);
  assert.throws(
    () => composeMission({ goal: '检查代币风险', contractAddress: '0x1234' }, catalog),
    /有效的 EVM 合约地址/,
  );
  assert.throws(
